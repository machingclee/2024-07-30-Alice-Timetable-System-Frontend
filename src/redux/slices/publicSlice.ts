...
const appSlice = createSlice({
    ...
    extraReducers: builder => {
        builder.addCase(PublicThunkAction.getClassesStatus.fulfilled, (state, action) => {
...
export class PublicThunkAction {
    public static getClassesStatus = createAsyncThunk(
        ...
        async (props: { pkgUUID: string }, api) => {
            const { pkgUUID } = props;
            const res = await apiClient.get<CustomResponse<TimetableClass[]>>(
                apiRoutes.GET_PACKAGE_CLASS_STATUS(pkgUUID)
            );
...
registerDialogAndActions(publicMiddleware, [
    ...
    {
        rejections: [PublicThunkAction.getClassesStatus.rejected],
    },
]);
