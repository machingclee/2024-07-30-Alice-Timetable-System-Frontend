...
const userSlice = createSlice({
    ...
    extraReducers: builder => {
        builder.addCase(UserThunkAction.getUsers.fulfilled, (state, action) => {
        ...
        builder.addCase(UserThunkAction.updateUser.fulfilled, (state, action) => {
...
export class UserThunkAction {
    ...
    public static getUsers = createAsyncThunk('userSlice/getUsers', async (_: undefined, api) => {
        const res = await apiClient.get<CustomResponse<User[]>>(apiRoutes.GET_USERS);
        return processRes(res, api);
    });
    public static updateUser = createAsyncThunk('userSlice/updateUser', async (props: User, api) => {
